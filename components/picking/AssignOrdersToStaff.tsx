...
function AssignOrdersToStaff() {
  const [orders, setOrders] = useState<OrderForPicking[]>([]);
  ...
  const [filterStatus, setFilterStatus] = useState<string>("ALLOCATED");
  ...
  const loadData = async () => {
    ...
    try {
      const [ordersRes, staffRes] = await Promise.all([
        fetch(`/api/orders?status=${filterStatus}`),
      ...
      const ordersData: OrderForPicking[] = await ordersRes.json();
  ...
  const handleCreatePickList = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const pickList: CreatePickListResponse = await response.json();
        alert(`Pick list ${pickList.batchNumber} created successfully!`);
        setSelectedOrders([]);
        setSelectedStaff("");
        loadData();
      } else {
        const error = await response.json();
        alert(error.error || "Failed to create pick list");
      }
    } catch (error) {
      console.error("Error creating pick list:", error);
      alert("Error creating pick list");
    } finally {
      setCreating(false);
    }
  };
    ...
    <div className="max-w-7xl mx-auto p-4">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Create pick lists and assign them to warehouse staff members
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="p-4 border-b border-gray-200 dark:border-zinc-700">
              <div className="flex gap-3">
                ...
                <select
                  ...
                  className="px-4 py-2 border border-gray-300 dark:border-zinc-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 dark:bg-zinc-700 dark:text-gray-100"
                >
        ...
        {/* Right Column - Pick List Creation */}
            ...
            {selectedOrders.length > 0 ? (
              ...
              <div className="bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-lg p-4 mb-4 text-center">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  Select orders from the list to create a pick list
                </p>
              </div>
            )}
                    ...
                    <div className="space-y-1">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium text-gray-900 dark:text-gray-100">
                          {staffMember.workload.activePickLists}
                        </span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-600 dark:text-gray-400">
                        ...
                        <span className="font-medium text-gray-900 dark:text-gray-100">
                          {staffMember.workload.remainingItems}
                        </span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-600 dark:text-gray-400">
...
export default AssignOrdersToStaff;
...
//                   .map((s) => (
//                     <option key={s.id} value={s.id}>
//                       {s.name} ({s.workload?.remainingItems || 0} items)
//                     </option>
//                   ))}
//               </select>
//             </div>
