...
export default function InvoiceForm({
  ...
  const [fees, setFees] = useState("0"); // ← NEW
  ...
  const generateSKU = (name: string, index: number): string => {
    ...
    // ✅ FIXED: Handle different word counts properly
    let prefix = "";

    if (words.length === 0) {
      prefix = "ITEM";
    } else if (words.length === 1) {
      // Single word: just use it (up to 8 chars)
      prefix = words[0].substring(0, 8);
    } else {
      // Multiple words: join first two with dash (up to 8 chars total)
      prefix = words
        .slice(0, 2)
  ...
  const updateItem = (
    ...
    if (field === "name" && newItems[index].autoGenerate) {
  ...
  const toggleSKUMode = (index: number) => {
    ...
    if (newItems[index].autoGenerate && newItems[index].name) {
      newItems[index].sku = generateSKU(newItems[index].name, index);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!uploadedFile) {
      toast({
        variant: "destructive",
        title: "Missing invoice image",
        description: "Please upload an image of the original invoice.",
      });
      return;
    }

    if (items.some((item) => !item.name || !item.sku)) {
      toast({
        variant: "destructive",
        title: "Incomplete items",
        description: "All items must have a name and SKU",
      });
      return;
    }
    ...
    try {
      ...
      formData.append("shipping", shipping); // ← NEW
      formData.append("fees", fees); // ← NEW
      formData.append(
        "items",
        JSON.stringify(
                        ...
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <div className="flex gap-2 text-xs">
                            <button
                              type="button"
                              onClick={() =>
                                updateItem(index, "autoGenerate", true)
                              }
                              className={`flex items-center gap-1 px-2 py-1 rounded border ${
                                item.autoGenerate
                                  ? "bg-blue-100 text-blue-600 border-blue-500 dark:bg-blue-800/20 dark:text-blue-400"
                                  : "text-gray-500 border-gray-300 dark:border-zinc-700 hover:text-blue-600"
                              }`}
                            >
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                updateItem(index, "autoGenerate", false)
                              }
                              className={`flex items-center gap-1 px-2 py-1 rounded border ${
                                !item.autoGenerate
                                  ? "bg-blue-100 text-blue-600 border-blue-500 dark:bg-blue-700/20 dark:text-blue-400"
                                  : "text-gray-500 border-gray-300 dark:border-zinc-800 hover:text-blue-600"
                              }`}
                            >
                              Manual
                            </button>
