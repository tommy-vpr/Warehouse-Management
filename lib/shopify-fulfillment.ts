...
type UpdateShopifyFulfillmentArgs = {
  ...
  trackingUrls?: string[]; // ✅ Array of tracking URLs (optional)
  lineItems: Array<{
    variantId?: string;
    sku?: string;
    quantity: number;
...
export async function updateShopifyFulfillment(
  ...
  // 3) Create fulfillment with MULTIPLE tracking numbers
  const CREATE = `
  ...
  // ✅ Build tracking info with multiple numbers
  const input = {
    lineItemsByFulfillmentOrder: foPlans,
    trackingInfo: {
      company: trackingCompany,
      numbers: trackingNumbers, // ✅ Array of tracking numbers
      ...(trackingUrls.length > 0 ? { urls: trackingUrls } : {}),
    },
    notifyCustomer,
  };
  ...
  console.log(
    `✅ Tracking numbers: ${payload.fulfillment.trackingInfo
      .map((t) => t.number)
      .join(", ")}`
  );
  ...
  return {
    ...
    trackingNumbers: payload.fulfillment.trackingInfo.map((t) => t.number),
  };
}
...
// type UpdateShopifyFulfillmentArgs = {
//   orderId: string;
//   trackingNumbers: string[]; // ✅ Array
//   trackingCompany?: string;
//   trackingUrls?: string[]; // ✅ Array
//   lineItems: Array<{
//     variantId?: string;
//     sku?: string;
//     quantity: number;
...
//     trackingInfo: {
//       company: trackingCompany,
//       numbers: trackingNumbers, // ✅ Use "numbers" (plural)
//       ...(trackingUrls?.length ? { urls: trackingUrls } : {}), // ✅ Use "urls" (plural)
//     },
//     notifyCustomer,
//   };
